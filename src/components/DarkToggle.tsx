    ...
    <ThemeToggler>
      {({ theme, toggleTheme }: Props) => {
        const [value, setValue] = React.useState(
          themes[themes.findIndex(({ value }) => value === theme)]
        )
        const handleChange = (theme: { value: string; label: string }) => {
          ...
          <div className="flex flex-row items-center justify-center m-auto">
            <Listbox value={value} onChange={handleChange}>
              <div className="relative">
                ...
                >
                  <Listbox.Options className="absolute py-1 mt-1 overflow-auto text-base rounded-md shadow-lg shadow-primario max-h-60 ring-1 ring-primario ring-opacity-5 focus:outline-none sm:text-sm">
